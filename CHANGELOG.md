...
- 构建 5：全软件采用固定顶部与底部、弹性内容区域；新增主页面、配置 API、检查更新、LR 插件与帮助快捷栏，支持 Alt+1～4 和 F1。长说明移至帮助，按钮支持延迟悬停提示；API 配置改为基本／高级标签页。
- 补齐人脸改为以当前人物搜索整个工作区的漏标照片，无需记忆组号；显示参考照片与候选缩略图，人工确认后应用。采用本地检测与颜色纹理排序，未校准真实识别准确率，不自动确认。
