...
AI 选片窗口分为 **API 提交、网页提交、人工复核** 三页，共用上方的任务和选片偏好。
网页提交支持 Ctrl / Shift 多选批次，或一键全选未完成批次，点击“合并准备所选批次”即可复制一份完整提示词并打开全部联系表目录。
在大模型网页一次上传目录中的联系表并粘贴提示词，收到回答后使用“导入整份回答”，自动分配回各批次，再到人工复核确认。
...
修改灵敏度后点“重新生成联系表”应用重新分组；仅更改每页数量和列数保留分组。人工分组被覆盖前会提示。
...
- 已有工作区升级后，启用技术筛查并点击“重新生成联系表”应用新规则；结果仍统一导出到 LR。
