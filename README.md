...
4. 进入“AI 选片与 LR 导出”后自动在后台准备新一轮全量初选，替换上一轮任务和 AI 结果；初筛弃置照片不进入 AI 图片或提示词。
...
- 每次进入选片窗口自动准备新一轮全量任务，不保留任务历史。重新提交也替换上一轮结果；当前轮仍支持失败后继续、网页回答导入及查看结果。
- 默认批次尽量保持完整组，约 24 张照片；超大组独立一批。“拆小本批重试”创建更小的新任务，可能拆组，提示词会注明比较范围。
- 分组成员、照片文件或裁切改变会使相关组结果过时；旧快照不能继续提交或套用回答。
- 主页面的已完成扫描、分组、日志及偏好会记忆；再次进入 AI 选片窗口会准备新一轮，已导出的 LR 文件仅在再次导出时替换。
